try:
    ...
    menu7.add_command(label='显示运行速度', command=speed)
    menu7.add_command(label='用pdb调试', command=lambda: MyThread(pdb_debug))
    menu7.add_command(label='用ipdb调试', command=lambda: MyThread(ipdb_debug))
    menubar.add_cascade(label='python调试', menu=menu7)
    top.config(menu=menubar)
    menu8 = Menu(menubar, tearoff=False)
    menu8.add_command(label='修改为1号风格（默认）', command=normal_theme)
    menu8.add_command(label='修改为2号风格', command=other_theme)
    menu8.add_command(label='修改为3号风格', command=third_theme)
    menu8.add_command(label='修改为4号风格', command=fourth_theme)
    menubar.add_cascade(label='语法高亮风格', menu=menu8)
    top.config(menu=menubar)
    menu9 = Menu(menubar, tearoff=False)
    menu9.add_command(label='显示目录下的文件', command=lambda: MyThread(dirdir))
    ...
    menu12.add_command(label='init', command=lambda: MyThread(git_init))
    menu12.add_command(label='add', command=lambda: MyThread(git_add))
    menu12.add_command(label='clone', command=lambda: MyThread(git_clone))
    menu12.add_command(label='commit', command=lambda: MyThread(git_commit))
