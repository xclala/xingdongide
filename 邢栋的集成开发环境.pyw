try:
    ...
    def python_run():
        global opened_file_path
        system("python " + opened_file_path + "& echo ------------------ & pause")
        top.title("用python运行" + opened_file_path)


    def c_compile():
        global opened_file_path
        system("gcc -O3 " + opened_file_path + "& echo ------------------ & pause")
        top.title("编译优化" + opened_file_path)


    def java_compile():
        global opened_file_path
        system("javac " + opened_file_path + "& echo ------------------ & pause")
        top.title("java编译" + opened_file_path)


    def java_run():
        global opened_file_path
        system("java " + opened_file_path + "& echo ------------------ & pause")
    ...
    def ipython_run():
        global opened_file_path
        system("ipython " + opened_file_path + "& echo ------------------ & pause")
    ...
    def process_key(key):
        current_line_num, current_col_num = map(int,
        ...
        if key.keycode == 13:
            ...
            current_line = contents.get(f'{current_line_num}.0',
            ...
            for line in lines:
                ...
                for index, ch in enumerate(line):
                    if ch == "'" and not flag2:
                        ...
                        if ch not in ascii_letters:
                            ...
                            if ch == '#':
                                contents.insert(INSERT, line[index:], 'comment')
                                break
                            else:
                ...
                if flag1:
                    ...
                    if word in bifs:
                        ...
                        contents.insert(INSERT, word)
            contents.mark_set('insert', f'{current_line_num}.{current_col_num}')
    ...
    menu1.add_command(label='隐藏文件',
                    command=run(["attrib", "+H", message.get()], shell=True))
    menu1.add_command(label='恢复隐藏的文件',
                    command=run(["attrib", "-H", message.get()], shell=True))
    menu1.add_command(label='只读文件',
                    command=run(["attrib", "+R", message.get()], shell=True))
    ...
    menu8.add_command(label='生成测试文件并运行', command=lambda: MyThread(coverage_run))
    menu8.add_command(label='显示结果', command=lambda: MyThread(coverage_report))
    menu8.add_command(label='生成html文件夹', command=lambda: MyThread(coverage_html))
